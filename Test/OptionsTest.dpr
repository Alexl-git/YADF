...
begin
  ...
  // The "Default: X" suffix is generated by OptionHint from DefaultOptions;
  // a hand-written copy in a table hint would drift from the real default.
  Bad := '';
  for i := 0 to High(T) do
    if T[i].Hint.Contains('Default:') then
      Bad := Bad + ' ' + T[i].Ident;
  Check('no hand-written Default: in table hints (bad:' + Bad + ')', Bad = '');
  Check('OptionHint appends the live default', OptionHint(T[0]).Contains('Default: '));
end;
...
// ReadIntIni must accept what a human plausibly writes in an ini (trailing
// comment, whitespace, hex) and fall back to the default for garbage --
// TIniFile.ReadInteger silently returned the default for all of these.
procedure TestLenientIntRead;
var
  Path: string;
  Ini : TIniFile;
begin
  Path := TPath.Combine(TPath.GetTempPath, 'yadf_intread_test.ini');
  TFile.WriteAllText(Path,
    '[Format]' + sLineBreak +
    'A=120 ; tuned down' + sLineBreak +
    'B= 42 ' + sLineBreak +
    'C=abc' + sLineBreak +
    'D=$10' + sLineBreak +
    'E=7 // note' + sLineBreak);
  ...
  try
    Check('int: trailing ; comment ignored', ReadIntIni(Ini, 'Format', 'A', -1) = 120);
    Check('int: padded value parsed',        ReadIntIni(Ini, 'Format', 'B', -1) = 42);
    Check('int: garbage -> default',         ReadIntIni(Ini, 'Format', 'C', -1) = -1);
    Check('int: $-hex parsed',               ReadIntIni(Ini, 'Format', 'D', -1) = 16);
    Check('int: trailing // comment ignored',ReadIntIni(Ini, 'Format', 'E', -1) = 7);
    Check('int: missing key -> default',     ReadIntIni(Ini, 'Format', 'Z', 99) = 99);
  finally
    Ini.Free;
    TFile.Delete(Path);
  end;
