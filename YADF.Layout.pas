...
begin
  LastNL := 0;
  for i := Length(APre) downto 1 do
    if (APre[i] = #10) or (APre[i] = #13) then
    begin
      LastNL := i;
      Break;
    end;
  ...
  if AOpts.LowercaseKeywords then
    ...
    begin
      ...
      IsLiteralKind := T.Kind in [
        ptIdentifier, ptIntegerConst, ptFloat,
        ptStringConst, ptStringDQConst, ptAsciiChar,
        ptAnsiComment, ptBorComment, ptSlashesComment,
        ptCRLF, ptCRLFCo, ptSpace, ptNull, ptUnknown,
        ptIfDirect, ptIfDefDirect, ptIfNDefDirect, ptIfOptDirect,
        ptElseDirect, ptElseIfDirect, ptEndIfDirect, ptIfEndDirect,
        ptDefineDirect, ptUndefDirect, ptIncludeDirect, ptResourceDirect,
        ptCompDirect, ptScopedEnumsDirect];
  ...
  if AOpts.UpperDirectives then
    ...
    begin
      ...
      if T.Kind in [
        ...
        ptElseDirect, ptElseIfDirect, ptEndIfDirect, ptIfEndDirect,
        ptDefineDirect, ptUndefDirect, ptIncludeDirect, ptResourceDirect,
        ptCompDirect, ptScopedEnumsDirect] then
      begin
...
var
  ...
  begin
    ...
    Result := L.StartsWith('begin') or L.StartsWith('case') or L.StartsWith('try')
           or L.StartsWith('asm') or L.StartsWith('record');
  end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          NextTrim := Trim(Next);
          if (NextTrim <> '')
             and (LeadingSpaces(Next) > LeadingSpaces(Cur))
             and not NextStartsCompound(NextTrim) then
          begin
...
begin
  T := TrimLeft(ALine);
  Result := SameText(Copy(T, 1, 5), 'const')
            and ((Length(T) = 5) or (T[6] = ' ') or (T[6] = #9) or (T[6] = #13));
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        MaxPos := Anchors[i];
        j := i + 1;
        while (j < Lines.Count) and (Anchors[j] > 0)
              and not StartsBlockBoundary(Lines[j]) do
        begin
        ...
        begin
          ...
          begin
            ...
            if Pad > 0 then
            begin
              Line := Copy(Lines[i], 1, Anchors[i] - 1)
                    + StringOfChar(' ', Pad)
                    + Copy(Lines[i], Anchors[i], MaxInt);
              Out_.Append(Line);
            end
            else
              Out_.Append(Lines[i]);
...
begin
  ...
  try
    ...
    begin
      ...
      if not (Tok in [
        ptSpace, ptCRLF, ptCRLFCo, ptIdentifier,
        ptStringConst, ptStringDQConst, ptIntegerConst, ptFloat, ptAsciiChar,
        ptAnsiComment, ptBorComment, ptSlashesComment]) then
      begin
        Shape.Add(Tok);
  ...
  try
    ...
    try
      ...
      begin
        ...
        j := i + 1;
        while (j < Lines.Count)
              and Info[j].HasAssign
              and ShapesMatch(Info[i].Shape, Info[j].Shape) do
          Inc(j);
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if Pad > 0 then
              begin
                WorkLine[L] := Copy(WorkLine[L], 1, WorkCols[L][k] - 1)
                              + StringOfChar(' ', Pad)
                              + Copy(WorkLine[L], WorkCols[L][k], MaxInt);
                for var M: Integer := k to High(WorkCols[L]) do
...
function ReflowLineBreaks(const S: string; AMaxLen: Integer): string;
  ...
  begin
    ...
    if Length(Trimmed) > Length(AWord) then
      Result := not (((Trimmed[Length(AWord) + 1] >= 'a') and (Trimmed[Length(AWord) + 1] <= 'z')) or
                     ((Trimmed[Length(AWord) + 1] >= 'A') and (Trimmed[Length(AWord) + 1] <= 'Z')) or
                     ((Trimmed[Length(AWord) + 1] >= '0') and (Trimmed[Length(AWord) + 1] <= '9')) or
  ...
  begin
    ...
    if EndPos > 1 then
      Result := not (((Trimmed[EndPos - 1] >= 'a') and (Trimmed[EndPos - 1] <= 'z')) or
                     ((Trimmed[EndPos - 1] >= 'A') and (Trimmed[EndPos - 1] <= 'Z')) or
                     ((Trimmed[EndPos - 1] >= '0') and (Trimmed[EndPos - 1] <= '9')) or
                     (Trimmed[EndPos - 1] = '_'))
    else
      Result := True;
  end;
...
function EnforceBlankLines(const S: string; const AOpts: TYadfOptions): string;
  ...
  begin
    Result := StartsWordCI(ALine, 'interface')
           or StartsWordCI(ALine, 'implementation')
           or StartsWordCI(ALine, 'initialization')
           or StartsWordCI(ALine, 'finalization');
  end;
  ...
  begin
    T := TrimLeft(ALine);
    Result := StartsWordCI(T, 'procedure')
           or StartsWordCI(T, 'function')
           or StartsWordCI(T, 'constructor')
           or StartsWordCI(T, 'destructor');
...
var
  ...
  Have:  Integer;
begin
  if (AOpts.BlanksBeforeSection <= 0)
     and (AOpts.BlanksBeforeMethod <= 0)
     and (AOpts.BlanksBeforeType <= 0) then
    Exit(S);
  Lines := TStringList.Create;
  try
    Lines.LineBreak := #13#10;
...
var
  ...
  var
    k: Integer;
  begin
    for k := Stack.Count - 1 downto 0 do
      case Stack[k] of
        ptCase: Exit(True);
  ...
  begin
    for k := 0 to Stack.Count - 1 do
      if Stack[k] in [ptClass, ptObject, ptInterface, ptRecord] then
        Exit(True);
    Result := False;
  end;
  ...
  begin
    ...
    for k := 0 to Stack.Count - 1 do
      if Stack[k] in [ptBegin, ptCase, ptTry, ptAsm] then
      begin
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          ...
          case CurLineLast of
            ptThen, ptDo, ptElse:
              BodyBonus := 1;
            ptColon:
              if StackContainsCaseAtTop then
                BodyBonus := 1;
            ptSemiColon:
              if ParensDepth = 0 then
                BodyBonus := 0;
            ptEnd, ptBegin, ptCase, ptRecord, ptTry, ptAsm, ptObject:
              BodyBonus := 0;
          end;
          CurLineLast := ptUnknown;
          ExpectSectionDecl := False;
        ...
        if ExpectSectionDecl
           and not (T.Kind in [ptAnsiComment, ptBorComment, ptSlashesComment]) then
        begin
        ...
        if AfterCRLF
           and (InUsesClause
                or (T.Kind in [ptPlus, ptMinus, ptStar, ptSlash, ptComma,
                                ptOr, ptAnd, ptXor, ptDiv, ptMod, ptShl, ptShr,
                                ptEqual, ptLower, ptGreater, ptLowerEqual,
                                ptGreaterEqual, ptNotEqual, ptDotDot, ptPoint,
                                ptThen, ptDo, ptOf])) then
        begin
        ...
        begin
          ...
          if T.Kind = ptEnd then
            EffectiveDepth := Max(0, EffectiveDepth - 1);
          if T.Kind in [ptExcept, ptFinally] then
            EffectiveDepth := Max(0, EffectiveDepth - 1);
          if (T.Kind in [ptType, ptVar, ptConst])
             and (StackTop in [ptType, ptVar, ptConst]) then
            EffectiveDepth := Max(0, EffectiveDepth - 1);
          if (T.Kind = ptBegin)
             and (not InClassOrRecord)
             and (StackTop in [ptType, ptVar, ptConst]) then
            EffectiveDepth := Max(0, EffectiveDepth - 1);
          if (T.Kind in [ptProcedure, ptFunction, ptConstructor, ptDestructor])
             and (not InClassOrRecord) then
          ...
          if T.Kind = ptUses then
            EffectiveDepth := 0;
          if InVisibility
             and not (T.Kind in [ptPrivate, ptPublic, ptProtected, ptPublished,
                                  ptStrict, ptEnd])
             and not (T.ExID in [ptPrivate, ptPublic, ptProtected, ptPublished,
                                  ptStrict]) then
            Inc(EffectiveDepth);
          if (BodyBonus > 0)
             and not (T.Kind in [ptBegin, ptCase, ptTry, ptAsm, ptEnd,
                                  ptElse, ptIf, ptRecord, ptObject]) then
            Inc(EffectiveDepth, BodyBonus);
          if (OpenProcRegions > 1)
             and not ((T.Kind in [ptProcedure, ptFunction, ptConstructor, ptDestructor])
                      and (not InClassOrRecord)) then
        ...
        begin
          case T.Kind of
            ptRoundOpen, ptSquareOpen:
              Inc(ParensDepth);
            ptRoundClose, ptSquareClose:
              if ParensDepth > 0 then Dec(ParensDepth);
            ptType, ptVar, ptConst:
              if (ParensDepth = 0)
                 and not (StackTop in [ptBegin, ptCase, ptTry, ptAsm]) then
              begin
                CloseSectionIfOpen;
                StackPush(T.Kind);
                InVisibility := False;
            ...
            ptProcedure, ptFunction, ptConstructor, ptDestructor:
            ...
            ptRecord, ptCase, ptTry, ptAsm:
              begin
                StackPush(T.Kind);
                InVisibility := False;
              end;
            ptClass, ptObject:
              if PrevNonKind = ptEqual then
              begin
                StackPush(T.Kind);
                InVisibility := False;
              end;
            ...
            ptPrivate, ptPublic, ptProtected, ptPublished:
              InVisibility := True;
            ptUses, ptContains, ptRequires:
              InUsesClause := True;
            ptSemiColon:
              if (ParensDepth = 0) and InUsesClause then
                InUsesClause := False;
            ptForward, ptExternal:
              if (not InClassOrRecord) and (PendingProcStack.Count > 0) then
              begin
                PendingProcStack.Delete(PendingProcStack.Count - 1);
                if OpenProcRegions > 0 then Dec(OpenProcRegions);
              end;
          ...
          if T.ExID in [ptPrivate, ptPublic, ptProtected, ptPublished] then
            InVisibility := True;
          if (T.ExID in [ptForward, ptExternal])
             and (not InClassOrRecord)
             and (PendingProcStack.Count > 0) then
          begin
            PendingProcStack.Delete(PendingProcStack.Count - 1);
            if OpenProcRegions > 0 then Dec(OpenProcRegions);
          end;
...
function CollectUsesItems(const ATokens: TTokenList; AStartIdx, AEndIdx: Integer;
  out AHasInClause: Boolean): TArray<string>;
var
  i:       Integer;
  Items:   TList<string>;
  CurText: string;
...
begin
  ...
  try
    ...
    begin
      ...
      if HasContent and (ATokens[i].Pre <> '')
         and (Sb.Length > 0) and (Sb.Chars[Sb.Length - 1] <> ' ') then
        Sb.Append(' ');
...
begin
  for i := AFrom to ATo do
    if not (ATokens[i].Kind in [ptSpace, ptCRLF, ptCRLFCo]) then
      ...
      begin
        ...
        begin
          while (k <= ACloseIdx - 1) and (ATokens[k].Kind = ptSpace) do
            Inc(k);
          if (k <= ACloseIdx - 1)
             and IsCommentKind(ATokens[k].Kind)
             and (ATokens[k].Line = CommaLine) then
          begin
...
begin
  ...
  begin
    Dec(Limit);
    K := ATokens[i].Kind;
    if K in [ptSpace, ptCRLF, ptCRLFCo,
             ptAnsiComment, ptBorComment, ptSlashesComment,
             ptIfDirect, ptIfDefDirect, ptIfNDefDirect,
             ptElseDirect, ptElseIfDirect, ptEndIfDirect, ptIfEndDirect] then
    begin
      Dec(i);
      Continue;
    end;
...
var
  ...
  begin
    for Child in G.Children do
      if Child.OpenIdx = AOpenIdx then
        Exit(Child);
    Result := nil;
  end;
    ...
    begin
      ...
      begin
        ...
        if Assigned(SubGroup)
           and (SubGroup.Kind in [gkParens, gkBrackets])
           and (SubGroup.CloseIdx = Items[i].Last)
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if (CurCol + InlineW <= AOpts.MaxLen)
                and not RangeHasMultiLineToken(Tokens, Child.OpenIdx, Child.CloseIdx) then
        begin
      ...
      begin
        Marker := Format('// TODO -oYADF : ''%s'' on line %d has no matching ''end''',
                          [Tokens[Child.OpenIdx].Text, Tokens[Child.OpenIdx].Line]);
        if Pos(Marker, ASource) = 0 then
          EmitText(#13#10 + Marker);
      end;
      if AOpts.LabelLongBlocks
         and (Child.Kind = gkBlock)
         and not Child.ForceClosed
         and (Child.CloseIdx > Child.OpenIdx) then
      begin
    ...
    try
      ...
      begin
        ...
        if (Line[i] = ',') and (Depth > 0)
           and (i + 1 <= Length(Line)) and (Line[i + 1] = ' ') then
        begin
          Positions.Add(i + 2);
          Inc(i);
          Continue;
